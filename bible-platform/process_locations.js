...
const translateVerse = (verseStr) => {
  // e.g. "Matt 4:13" or "1 Kgs 5:12"
  let result = verseStr.replace(/([1-3]?\s?[A-Za-z]+)\s(\d+:\d+)/, (match, book, chapVerse) => {
    let cleanBook = book.replace(/\s/g, ''); // "1 Kgs" -> "1Kgs"
...
async function processData() {
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      const typeKr = {
        'settlement': '도시/마을', 'mountain': '산', 'mountain range': '산맥', 'river': '강', 'lake': '호수', 'sea': '바다',
