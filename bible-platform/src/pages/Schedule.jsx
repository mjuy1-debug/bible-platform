...
const Schedule = () => {
  ...
  const handleAddEvent = () => {
    ...
    const payload = {
      ...
      description: newEvent.description ? newEvent.description.trim() : undefined,
    };
              ...
              {selectedDateEvents.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                  {selectedDateEvents.map(e => (
                    <EventCard key={e.id} event={e} onDelete={deleteEvent} onEdit={openEditForm} />
                  ))}
                </div>
              )}
            </motion.div>
