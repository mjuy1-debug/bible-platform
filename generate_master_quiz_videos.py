# -*- coding: utf-8 -*-
# generate_master_quiz_videos.py
import json
import re
import sys
...
print(f"Loaded {len(quiz_list)} quiz sets.")

# 52주차 주간 통독 골든벨 고유 매핑 (Week 1 ~ Week 52)
WEEK_VIDEOS = {
  1: {
...
for item in quiz_list:
    ...
    if cat == '👑 성경 인물 열전' or q_id.startswith('hero_char_'):
        ...
        videos = [
            {
                'title': f'👑 [{char_name}]의 믿음과 생애 풀 스토리',
                'summary': f'성경 인물 {char_name}의 부르심과 순종, 고난과 승리의 신앙 여정',
                'points': [
                    f'{char_name}의 생애 속 결정적 신앙 선택과 믿음의 결단',
                    desc or f'{char_name}을 통하여 드러난 하나님의 신실하신 약속 성취'
                ],
                'searchQuery': f'성경 인물 {char_name} 이야기 설교 애니메이션'
            },
            {
                'title': f'📖 바이블프로젝트: [{char_name}]과 구속사의 비밀',
                'summary': f'{char_name}의 사건이 예수 그리스도의 십자가와 구원 복음 속에서 갖는 구속사적 의미',
                'points': [
                    f'{char_name}과 관련된 핵심 성경 구절 및 족보 퀴즈 포인트',
                    '성경 본문 속에 나타난 하나님의 언약과 오늘 우리를 향한 교훈'
                ],
                'searchQuery': f'바이블프로젝트 {char_name} 구속사 {book}'
            }
        ]
        ...
        videos = [
            {
                'title': f'📜 구약 입체 탐구: [{topic_name}] 핵심 사건',
                'summary': f'{desc or f"{book} 본문 속 주요 사건과 구속사적 계시"}',
                'points': [
                    f'본문 범위: {q_range or book}',
                    f'{topic_name}에 나타난 하나님의 주권과 구원 역사'
                ],
                'searchQuery': f'바이블프로젝트 {book} 개요 {topic_name}'
            },
            {
                'title': f'💡 [{topic_name}] 성경 퀴즈 100점 만점 족보',
                'summary': f'{topic_name} 본문에 등장하는 핵심 인물, 지명, 사건의 영적 의미 총정리',
                'points': [
                    f'{book} 말씀 속 순종과 축복의 교훈',
                    '성경 본문의 핵심 질문과 정답 족보'
                ],
                'searchQuery': f'구약 성경 {book} {topic_name} 강해'
            }
        ]
        ...
        videos = [
            {
                'title': f'✝️ 신약 복음의 빛: [{topic_name}]',
                'summary': f'{desc or f"{book} 복음서와 서신서 속 예수 그리스도의 십자가와 부활, 제자도"}',
                'points': [
                    f'본문 범위: {q_range or book}',
                    f'{topic_name}을 통한 예수 그리스도의 은혜와 하나님 나라의 도래'
                ],
                'searchQuery': f'바이블프로젝트 {book} 개요 {topic_name}'
            },
            {
                'title': f'✨ [{topic_name}] 제자도 실천과 핵심 퀴즈 포인트',
                'summary': f'{topic_name} 말씀이 전하는 구원의 복음과 성도의 삶의 실천 지침',
                'points': [
                    f'{book} 본문의 주요 비유와 기적, 교훈',
                    '성경 퀴즈 만점을 위한 핵심 구절 암송'
                ],
                'searchQuery': f'신약 성경 {book} {topic_name} 강해'
            }
        ]
        ...
        videos = [
            {
                'title': f'🌟 어린이를 위한 애니메이션: [{topic_name}]',
                'summary': f'쉽고 재미있게 배우는 {topic_name} 이야기와 하나님의 크신 사랑',
                'points': [
                    f'{topic_name} 이야기의 재미있는 줄거리와 등장인물',
                    desc or '어린이와 초신자를 위한 쉽고 은혜로운 성경 교훈'
                ],
                'searchQuery': f'어린이 성경 동화 {topic_name} 히즈쇼 애니메이션'
            },
            {
                'title': f'💡 [{topic_name}] 주일학교 쏙쏙 성경 퀴즈',
                'summary': f'{topic_name} 이야기 속에 숨겨진 하나님의 약속과 믿음의 영웅들',
                'points': [
                    '퀴즈에 나오는 주요 단어와 인물 맞추기',
                    '하나님께 감사하고 순종하는 착한 마음'
                ],
                'searchQuery': f'주일학교 성경학교 {topic_name} 말씀 퀴즈'
            }
        ]
        ...
        videos = [
            {
                'title': f'📖 예수님의 기적과 비유: [{topic_name}]',
                'summary': f'{desc or f"예수님께서 베푸신 {topic_name}의 영적 의미와 하나님 나라의 비밀"}',
                'points': [
                    f'{topic_name} 본문의 역사적 배경과 표적의 기적 순간',
                    '비유 속에 담긴 천국의 비밀과 회개의 메시지'
                ],
                'searchQuery': f'예수님의 비유 기적 {topic_name} 바이블프로젝트'
            },
            {
                'title': f'🕊️ [{topic_name}] 속 십자가 사랑과 믿음의 고백',
                'summary': f'{topic_name}을 통해 질병을 고치시고 영혼을 구원하신 예수님의 긍휼과 사랑',
                'points': [
                    '기적을 체험한 사람들의 믿음의 고백과 변화',
                    '오늘 우리 삶에 역사하시는 주님의 치유와 회복'
                ],
                'searchQuery': f'예수님 말씀 {topic_name} 설교 애니메이션'
            }
        ]
        ...
        videos = [
            {
                'title': f'🙏 기도의 용사: [{topic_name}]의 부르짖음',
                'summary': f'{desc or f"간절한 부르짖음으로 하나님의 보좌를 움직인 {topic_name}의 기도"}',
                'points': [
                    f'{topic_name}의 기도의 배경과 응답의 축복',
                    '낙망하지 않고 끈질기게 매달린 기도의 본'
                ],
                'searchQuery': f'성경 인물 기도 {topic_name} 설교 말씀'
            },
            {
                'title': f'🔥 [{topic_name}] 기도의 능력과 승리의 비결',
                'summary': f'고난과 절망의 순간 기도로 역전승을 거둔 믿음의 거장들의 무릎 신앙',
                'points': [
                    '하나님의 뜻에 합당한 기도의 방법과 영적 원리',
                    '응답받는 기도의 3대 원리와 순종'
                ],
                'searchQuery': f'기도의 용사들 {topic_name} 바이블프로젝트'
            }
        ]
        ...
        videos = [
            {
                'title': f'🎉 교회력과 거룩한 절기: [{topic_name}]',
                'summary': f'{desc or f"{topic_name}의 성경적 기원과 구속사적 의미, 교회의 전통"}',
                'points': [
                    f'{topic_name}의 성경 본문과 구약의 예표',
                    '그리스도 중심의 거룩한 기념과 헌신'
                ],
                'searchQuery': f'교회력 절기 {topic_name} 의미 바이블프로젝트'
            },
            {
                'title': f'🕯️ [{topic_name}] 묵상과 찬양의 기쁨',
                'summary': f'{topic_name} 절기를 맞이하여 성도들이 지켜야 할 거룩한 신앙의 태도',
                'points': [
                    '절기 퀴즈에 나오는 주요 성경 구절과 의미',
                    '온 가족이 함께 나누는 감사와 찬양'
                ],
                'searchQuery': f'성경 절기 설교 {topic_name}'
            }
        ]
        ...
        videos = [
            {
                'title': f'🌈 성경 핵심 교리 특강: [{topic_name}]',
                'summary': f'{desc or f"기독교 신앙의 뼈대를 이루는 {topic_name}의 성경적 근거와 구원 진리"}',
                'points': [
                    f'{topic_name}에 관한 핵심 성경 구절 총정리',
                    '이단과 오류를 분별하는 정통 기독교 신앙'
                ],
                'searchQuery': f'바이블프로젝트 기독교 교리 {topic_name}'
            },
            {
                'title': f'📖 [{topic_name}] 말씀 마스터 퀴즈 족보',
                'summary': f'{topic_name} 교리가 성도의 일상 삶과 구원에 미치는 영적 영향과 적용',
                'points': [
                    '교리 퀴즈 만점을 위한 핵심 요약',
                    '성경 66권을 관통하는 거룩한 복음의 진리'
                ],
                'searchQuery': f'성경 말씀 테마 교리 {topic_name} 강해'
            }
        ]
        ...
        videos = [
            {
                'title': f'🗺️ 성지 탐험 랜선 투어: [{topic_name}]',
                'summary': f'{desc or f"성경 역사의 주 무대가 된 {topic_name}의 지리적 위치와 고고학적 유적"}',
                'points': [
                    f'{topic_name}에서 일어난 구약과 신약의 중요 사건들',
                    '성경 지도와 입체 지형으로 보는 생생한 성지'
                ],
                'searchQuery': f'성경 지리 성지순례 {topic_name} 바이블프로젝트'
            },
            {
                'title': f'🏛️ [{topic_name}] 성경 문화와 역사적 배경',
                'summary': f'성경 시대의 풍습, 화폐, 도량형, 절기와 함께 이해하는 {topic_name}',
                'points': [
                    '성경 지리 퀴즈 핵심 출제 포인트',
                    '말씀을 입체적으로 깨닫는 성경 배경 지식'
                ],
                'searchQuery': f'성경 역사 지리 문화 {topic_name} 탐구'
            }
        ]
        ...
        videos = [
            {
                'title': f'✨ [{topic_name}] 성경 말씀 가이드',
                'summary': desc or f'{topic_name} 성경 본문 말씀을 깊이 묵상하고 문제를 풀어보세요!',
                'points': [f'{book} 본문의 구속사적 은혜', '성경 퀴즈 핵심 요약'],
                'searchQuery': f'바이블프로젝트 {book} {topic_name}'
            }
        ]
    ...
    ALL_SET_VIDEOS[q_id] = {
        ...
        'relationReason': f'🌟 [{clean_title} 직결 맞춤 영상 리스트]',
...
print("Master generator complete! Successfully created rich bibleProjectVideos.js")
