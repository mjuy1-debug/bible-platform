# -*- coding: utf-8 -*-
# generate_multi_bible_videos.py

code = '''// src/data/bibleProjectVideos.js
...
// 1. 52주 성경 통독 골든벨 전 주차별 100% 직결 다중 영상 매핑 (2~4개 핵심 영상 리스트)
...
// 2. 성경 주요 90명 전 인물별 직결 스토리 영상 & 족보 가이드
export const CHARACTER_VIDEOS_MAP = {
  ...
  '솔로몬': {
    title: '솔로몬: 일천번제와 성전 건축, 지혜의 왕',
    summary: '기브온 산당에서 듣는 마음(지혜)을 구하고 7년 만에 예루살렘 성전을 완공한 지혜의 왕',
    points: ['일천번제 후 구한 듣는 마음과 재판의 지혜', '예루살렘 성전 건축과 봉헌 기도', '잠언, 전도서, 아가서의 저자'],
    searchQuery: '성경 인물 솔로몬 지혜 성전 건축 이야기'
  },
  '엘리야': {
    title: '엘리야: 갈멜산의 불과 세미한 음성',
    summary: '바알 선지자 450명과 대결하여 하늘에서 불을 내리고 호렙산 세미한 음성 속에서 새 사명을 받은 선지자',
    points: ['그릿 시냇가 까마귀와 사르밧 과부의 기름병 기적', '갈멜산 번제단에 임한 여호와의 불', '회오리바람을 타고 불수레와 불말과 함께 승천'],
    searchQuery: '성경 인물 엘리야 갈멜산 불 이야기'
  },
  '베드로': {
    title: '베드로: 수제자의 신앙고백과 순교',
    summary: '"주는 그리스도시요 살아계신 하나님의 아들이시니이다" 고백 후 세 번 부인했으나 부활의 주님을 만나 초대교회의 기둥이 됨',
    points: ['갈릴리 어부에서 사람을 낚는 어부로 부름받음', '가이사랴 빌립보의 위대한 신앙고백(마 16:16)', '오순절 설교로 3천 명을 회개시킴'],
    searchQuery: '성경 인물 베드로 신앙고백 사도행전 이야기'
  },
  '바울': {
    title: '사도 바울: 다메섹 도상의 회심과 이방인의 사도',
    summary: '교회를 핍박하던 사울에서 부활하신 예수님을 만나 3차 전도여행과 13권의 서신서를 남긴 위대한 이방인의 사도',
    points: ['다메섹 도상에서 강렬한 빛 가운데 회심함', '빌립보 감옥에서의 찬양과 지진의 기적', '"나의 달려갈 길과 주 예수께 받은 사명을 마치려 함에는"(행 20:24)'],
...
export const BIBLE_BOOK_GUIDE = {
  '창세기': { title: '창세기: 태초의 천지창조, 타락과 4대 족장(아브라함, 이삭, 야곱, 요셉)의 언약', query: '바이블프로젝트 창세기 개요' },
...
export function getBibleProjectVideos(quiz) {
  ...
  if (weekMatch) {
    ...
    if (WEEKLY_VIDEOS_MAP[weekNum]) {
      ...
      return {
        ...
        relationReason: `🌟 [${wData.weekTitle} 핵심 영상 & 족보 가이드]`,
  ...
  if (matchedCharVideos.length > 0) {
    return {
      ...
      relationReason: `👑 [${rawTitle} 인물 핵심 스토리 & 퀴즈 가이드]`,
...
print("Successfully generated rich bibleProjectVideos.js")
